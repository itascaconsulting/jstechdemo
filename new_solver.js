...
var u_r = function (p_i,r) {
    ...
    var delta2 = Math.PI/4.0 + phi/2.0;
    var Q = Math.tan(delta)/(Math.tan(delta-phi))-1;
    var denom = (1+Math.tan(delta)**2)*(p_i + c*math.cot(phi));
    var R = r0 * ((2*p0-sigma_cm+(1+Math.tan(delta)**2)*c*math.cot(phi))/denom)**(1/Q);
    var b = ((Math.tan(delta)**2-1)*p0+sigma_cm)/(Math.tan(delta)**2+1)*R**2;
    var t0 = p0+c*math.cot(phi)-(p_i + c*math.cot(phi)*(R/r0)**Q);
...
//r.map(function (d) { return u_r(d,r0).u; })._data;
